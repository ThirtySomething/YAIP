...
SCENARIO("Processing of datatype [short]", "[net::derpaul::yaip::YAIP]")
{
	...
	GIVEN("An empty instance of the YAIP parser")
	{
		...
		REQUIRE(true == sut.SectionKeyValueSet(S_SECTION_SHORT, S_KEY_SHORT, VALUE_SHORT));
		...
		WHEN("Save ini file")
		{
			REQUIRE(true == sut.INIFileSave(S_FILE_INI_SHORT));

			THEN("File exists")
			{
				REQUIRE(true == sut.INIFileExist(S_FILE_INI_SHORT));
			}
		}
		...
		{
			REQUIRE(true == sut.INIFileExist(S_FILE_INI_SHORT));
			REQUIRE(true == sut.INIFileLoad(S_FILE_INI_SHORT));
		...
		{
			REQUIRE(true == sut.INIFileExist(S_FILE_INI_SHORT));
			REQUIRE(true == sut.INIFileDelete(S_FILE_INI_SHORT));
