...
SCENARIO("Test YAIP with datatype unsigned short", "[unsigned short]")
{
	...
	GIVEN("An empty instance of the YAIP parser")
	{
		...
		{
			REQUIRE(sut.SectionKeyValueClear(S_SECTION_UNSIGNED_SHORT, S_KEY_UNSIGNED_SHORT));
		...
		WHEN("Delete key")
		{
			REQUIRE(sut.SectionKeyKill(S_SECTION_UNSIGNED_SHORT, S_KEY_UNSIGNED_SHORT));
